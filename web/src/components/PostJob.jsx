...
import { Plus, Sparkles, Loader2, AlertCircle, ArrowLeft, MapPin, DollarSign, Clock, Type } from 'lucide-react';
  ...
  const onSubmit = async (data) => {
    try {
      ...
      toast.success('Đăng tin tuyển dụng thành công!', {
        ...
        icon: '🚀',
        style: {
          borderRadius: '1.5rem', background: '#0F172A', color: '#fff', padding: '1.5rem', fontWeight: 'bold',
        },
      });
      reset();
  ...
  return (
    <div className="w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-4xl font-black text-primary-dark tracking-tighter">Đăng Tin tuyển dụng</h2>
          <p className="text-slate-500 font-bold mt-1 uppercase text-[10px] tracking-widest">Giao diện chuyên nghiệp cho nhà tuyển dụng</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-12 rounded-[3.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 space-y-10">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <ReactQuill
                    ...
                    placeholder="Viết yêu cầu ứng viên tại đây..."
                  />
                )}
              />
        ...
        >
          {postJobMutation.isPending ? <Loader2 className="animate-spin" /> : <Sparkles className="w-6 h-6" />}
